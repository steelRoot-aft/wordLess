...
export const bonusRouter = router({
  ...
  openRandom: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const unopenedLetters = word
        .split("")
        .filter((letter) => !alreadyOpened.includes(letter));
      ...
      return {
        success: true,
        bonusChar:
          unopenedLetters[Math.floor(Math.random() * unopenedLetters.length)],
        coins: user.coins,
      };
    }),
});
